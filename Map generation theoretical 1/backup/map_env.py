...
# --- Removed road connectivity (no ROAD tile anymore) ---
...
def compute_chunk_reward(grid, left_col=None, top_row=None):
    ...
    # --- removed road connectivity reward ---
...
def compute_overlap_reward(current_chunk, left_chunk=None, top_chunk=None, border=2):
    ...
    if left_chunk is not None:
        overlap_left = current_chunk[:, :border]
        overlap_right = left_chunk[:, -border:]
        reward += np.sum(overlap_left == overlap_right) * 0.3
        reward -= np.sum(overlap_left != overlap_right) * 0.6

    if top_chunk is not None:
        overlap_top = current_chunk[:border, :]
        overlap_bottom = top_chunk[-border:, :]
...
class ChunkEnv:
    ...
    def observe(self):
        # --- old feature-vector observation removed ---
        ...
        # --- new: return full grid as float in [0,1] ---
...
class MapEnvironment:
    def __init__(self, size=2500, chunk_size=50):
        ...
        # --- old: feature-vector state_size ---
        # self.state_size = 14
        # --- new: full-grid observation ---
    ...
    def step(self, action:int):
        ...
        # combine chunk reward with overlap reward
        left_chunk = None
        top_chunk = None
        if cj > 0:
            left_chunk = self.chunks[-2]
        if ci > 0:
            top_chunk = self.chunks[(ci - 1) * self.chunks_per_row + cj]
